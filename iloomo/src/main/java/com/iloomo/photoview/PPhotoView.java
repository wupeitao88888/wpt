...
public class PPhotoView extends ImageView {
    ...
    private void initBase() {
        ...
        if (mFromPInfo != null && System.currentTimeMillis() - mInfoTime < MAX_ANIM_FROM_WAITE) {
            animaFrom(mFromPInfo);
        }
...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private Runnable mClickRunnable = new Runnable() {
        ...
        public void run() {
            if (mClickListener != null) {
                mClickListener.onClick(PPhotoView.this);
            }
        }
    };
    ...
    private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            if (mLongClick != null) {
                mLongClick.onLongClick(PPhotoView.this);
            }
        }
...
//            onUp(e2);
    ...
    public PInfo getInfo() {
        ...
        return new PInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees, mScaleType);
    }

    public static PInfo getImageViewInfo(ImageView imgView) {
        int[] p = new int[2];
        ...
        return new PInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0, imgView.getScaleType());
    }
     ...
     */
    public void animaFrom(PInfo PInfo) {
        if (isInit) {
            reset();

            PInfo mine = getInfo();

            float scaleX = PInfo.mImgRect.width() / mine.mImgRect.width();
            float scaleY = PInfo.mImgRect.height() / mine.mImgRect.height();
            float scale = scaleX < scaleY ? scaleX : scaleY;

            float ocx = PInfo.mRect.left + PInfo.mRect.width() / 2;
            float ocy = PInfo.mRect.top + PInfo.mRect.height() / 2;

            mAnimaMatrix.reset();
            mAnimaMatrix.postTranslate(-mBaseRect.left, -mBaseRect.top);
            mAnimaMatrix.postTranslate(ocx - mBaseRect.width() / 2, ocy - mBaseRect.height() / 2);
            mAnimaMatrix.postScale(scale, scale, ocx, ocy);
            mAnimaMatrix.postRotate(PInfo.mDegrees, ocx, ocy);
            executeTranslate();
            ...
            mTranslate.withRotate((int) PInfo.mDegrees, 0);

            if (PInfo.mWidgetRect.width() < PInfo.mImgRect.width() || PInfo.mWidgetRect.height() < PInfo.mImgRect.height()) {
                float clipX = PInfo.mWidgetRect.width() / PInfo.mImgRect.width();
                float clipY = PInfo.mWidgetRect.height() / PInfo.mImgRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                ClipCalculate c = PInfo.mScaleType == ScaleType.FIT_START ? new START() : PInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
    ...
    public void animaTo(PInfo PInfo, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
            ...
            float tcx = PInfo.mRect.left + PInfo.mRect.width() / 2;
            float tcy = PInfo.mRect.top + PInfo.mRect.height() / 2;
            ...
            // 缩放
            float scaleX = PInfo.mImgRect.width() / mBaseRect.width();
            float scaleY = PInfo.mImgRect.height() / mBaseRect.height();
            float scale = scaleX > scaleY ? scaleX : scaleY;
            ...
            mTranslate.withRotate((int) mDegrees, (int) PInfo.mDegrees, mAnimaDuring * 2 / 3);

            if (PInfo.mWidgetRect.width() < PInfo.mRect.width() || PInfo.mWidgetRect.height() < PInfo.mRect.height()) {
                float clipX = PInfo.mWidgetRect.width() / PInfo.mRect.width();
                float clipY = PInfo.mWidgetRect.height() / PInfo.mRect.height();
                clipX = clipX > 1 ? 1 : clipX;
                clipY = clipY > 1 ? 1 : clipY;

                final float cx = clipX;
                final float cy = clipY;
                final ClipCalculate c = PInfo.mScaleType == ScaleType.FIT_START ? new START() : PInfo.mScaleType == ScaleType.FIT_END ? new END() : new OTHER();
